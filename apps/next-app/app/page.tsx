...
export default function HomePage() {
  ...
  // La Nuit de l'Info a lieu en France depuis 2007
  ...
  // Position du bouton selon l'étape des clics
  ...
  const handleGoldbergClick = () => {
    // 1 - Les 4 premiers clics : on voyage dans les coins
    if (clickStep < 4) {
      setClickStep((prev) => prev + 1);
      return;
    }

    // 2 - Ensuite, on ouvre la popup pour les questions débiles
    if (!showModal) {
      setShowModal(true);
    }
  };
  ...
  const handleLogicSubmit = (e: React.FormEvent) => {
    ...
    // On ne valide rien, l'objectif est juste de passer les 3 questions
    if (questionIndex < logicQuestions.length - 1) {
      setQuestionIndex((prev) => prev + 1);
    } else {
      // Après la 3e question, on passe à la phase "chercher sur le net"
  ...
  const closeModal = () => {
    // On ferme juste la popup, le bouton reste en bas à droite (clickStep = 4)
    setShowModal(false);
  };
